...
@section('content')
    ...
    <section class="mt-14 border-t border-base-300 pt-8" aria-labelledby="integration-heading"><h2 id="integration-heading" class="text-2xl font-semibold">Stable v6 integration</h2><p class="mt-3 max-w-3xl leading-7 text-base-content/75">This demo installs Laravel Daisy Kit <code>{{ \Composer\InstalledVersions::getPrettyVersion('art35rennes/laravel-daisy-kit') }}</code> from Git VCS at <code>{{ \Composer\InstalledVersions::getReference('art35rennes/laravel-daisy-kit') }}</code>. Every example uses only the published component and its corresponding ESM and CSS entry through the official <code>@daisy-kit</code> Vite alias.</p></section>
@endsection
