@php
    $reviewers = [
        ...
        ['value' => 'margaret', 'label' => 'Margaret Hamilton', 'description' => 'margaret.hamilton@example.test', 'initials' => 'MH', 'meta' => 'Quality'],
    ];
    $blade = <<<'BLADE'
<form method="get" action="{{ route('docs.module', 'combobox') }}" class="space-y-4">
    <x-daisy-kit::combobox name="reviewers" label="Reviewers" :options="$reviewers" :value="['ada']" :multiple="true" :max-suggestions="20" placeholder="Add a reviewer" />
...
component.setOptionRenderer((option) => {
    ...
    <article class="max-w-5xl">
        ...
        <section class="mt-10 space-y-6" aria-labelledby="combobox-examples-heading">
            ...
            <section id="combobox-example-1" class="min-w-0 rounded-box border border-base-300 bg-base-100 p-5">
                ...
                <p class="mb-5 mt-2 text-sm text-base-content/70">Click the field to see people, e-mail addresses and teams. Selected reviewers remain as compact tokens; typing ranks the list.</p>
                <form method="get" action="{{ route('docs.module', 'combobox') }}" class="space-y-4">
                    <x-daisy-kit::combobox name="reviewers" label="Reviewers" :options="$reviewers" :value="['ada']" :multiple="true" :max-suggestions="20" placeholder="Add a reviewer" />
                    <button type="submit" class="btn btn-primary">Assign reviewers</button>
                </form>
        ...
        <section class="mt-10 space-y-3">
            ...
            <p class="leading-7">getValue(), setValue(value), clear(), open(), close(), setOptionRenderer(renderer), clearOptionRenderer(); refresh(): Promise&lt;boolean&gt;.</p>
            ...
            <p class="leading-7">Options accept label, description, avatar, initials and meta as escaped data. Remote GET sources return { items, nextCursor? }. Superseded requests are cancelled; invalid responses emit an error.</p>
        </section>
